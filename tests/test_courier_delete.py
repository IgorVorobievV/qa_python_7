...
class TestDeleteCourier:  
    ...
    @pytest.mark.parametrize('id, text', [['', {"code":400,"message":"Недостаточно данных для удаления курьера."}],[1, {"code":404,"message":"Курьера с таким id нет."}]])
    def test_delete_courier_wrong_data_right_error(self, id, text):
        ...
        with allure.step(f"Проверка, что ответ возвращает соответствующую ошибку."):
    ...
    def test_delete_courier_without_id_error(self):
        ...
        with allure.step("Проверка, что вернётся ошибка."):
            assert response.text == '{"code":400,"message":"Недостаточно данных для удаления курьера."}'

    #если отправить запрос с несуществующим id, вернётся ошибка
    @allure.title('Проверить, что если отправить запрос с несуществующим id, вернётся ошибка')
    @allure.description('Проверить, что ответ содержит ')
