...
class TestOrderMake: 
    ...
    def test_order_make_one_color_success_order(self, data, order_make_and_delete):
        ...
        with allure.step("Возвращаем track заказа в фикстуру."):
            order_make_and_delete.append(response.json()['track'])
    ...
    @allure.description('Проверить, что код ответа 201')
    def test_order_make_two_color_success_order(self, order_make_and_delete):
    ...
    @allure.description('Проверить, что код ответа 201')
    def test_order_make_no_color_success_order(self, order_make_and_delete):
    ...
    def test_order_make_valid_order_track(self, order_make_and_delete):
        with allure.step("Серилизуем данные заказа."):
            payload = json.dumps(order_valid_data)
        with allure.step("Отправляем запрос на создание заказа и сохраняем ответ в переменную response."):
            response = request_order_make(payload)
